...
from .study_design import (GAUGE_BY_BLOCK, N_MISSES_PER_BLOCK,
                           N_NEARMISS_PER_BLOCK, PARTICIPANT_ORDERS,
                           TRUE_RELIABILITY_PCT)
...
EFFECTS: dict[str, dict] = {
    ...
    "workload": {"base": 4.5, "form": {"F1": 1.0, "F2": -0.2, "F3": -0.1},
                 "sd": 0.6, "p_sd": 0.7},
    # H2 calibration sliders: absolute error smaller (answers nearer truth) under
    # the contrastive forms. We model the slider *error magnitude* per form.
    "mm_misses_sd": {"base": 1.6, "form": {"F1": 0.8, "F2": -0.5, "F3": -0.6}},
    "mm_closecalls_sd": {"base": 1.6, "form": {"F1": 0.8, "F2": -0.5, "F3": -0.6}},
    "mm_reliability_sd": {"base": 16.0, "form": {"F1": 8.0, "F2": -5.0, "F3": -6.0}},
    # H2 recognition: probability of answering on the correct side of the slider.
    "mm_rec_p_correct": {"base": 0.74, "form": {"F1": -0.15, "F2": 0.09, "F3": 0.12},
                         "p_sd": 0.10},
...
def _resample_genericscales(seg_rows, form, letter, model, brng, prng):
    ...
    rec_off = model._p_offset("mm_rec_p_correct", prng)
    ...
    for r in seg_rows:
        ...
        # --- H2 calibration sliders ---
        elif re.match(r"^MM_[ABC]_q_misses$", addr):
            sd = max(0.3, EFFECTS["mm_misses_sd"]["base"] + model._delta("mm_misses_sd", form))
            set_addr(r, _fmt(_clip(N_MISSES_PER_BLOCK + brng.gauss(0, sd), 0, 9)))
        elif re.match(r"^MM_[ABC]_q_closecalls$", addr):
            sd = max(0.3, EFFECTS["mm_closecalls_sd"]["base"]
                     + model._delta("mm_closecalls_sd", form))
            set_addr(r, _fmt(_clip(N_NEARMISS_PER_BLOCK + brng.gauss(0, sd), 0, 9)))
        elif re.match(r"^MM_[ABC]_q_reliability$", addr):
            sd = max(2.0, EFFECTS["mm_reliability_sd"]["base"]
                     + model._delta("mm_reliability_sd", form))
            set_addr(r, _fmt(_clip(TRUE_RELIABILITY_PCT + brng.gauss(0, sd), 0, 100)))

        # --- H2 recognition sliders (0..100, threshold 50) ---
        elif _REC_RE.match(addr):
            m = _REC_RE.match(addr)
            blk, gtype, num = m.group(1), m.group(2), m.group(3)
            gauge = f"{'scales' if gtype == 'scale' else 'lights'}-{num}"
            truth_yes = gauge in GAUGE_BY_BLOCK.get(blk, ())
            p = _clip(EFFECTS["mm_rec_p_correct"]["base"]
                      + model._delta("mm_rec_p_correct", form) + rec_off, 0.02, 0.98)
            correct = brng.random() < p
            answer_yes = truth_yes if correct else (not truth_yes)
            set_addr(r, _fmt(brng.uniform(55, 100) if answer_yes else brng.uniform(0, 45)))
...
def _self_check(out_dir: Path) -> None:
    from .discovery import find_study_sessions
    from .validation import validate_sessions
    from .aggregate import build_metrics_table
